...
流是一种渐进返回数据的方式。普通响应通常要等完整结果生成后再处理，流式响应则会在请求开始后持续返回数据片段，直到发送结束信号。

对大模型应用来说，模型产生首个可用增量后，服务端就可以把它返回给前端，不必等整段回答全部生成完。前端拿到片段后立刻渲染，用户就会看到答案逐步出现。

Agent 的等待时间不只来自模型生成，还来自检索、工具调用、权限判断、代码执行和外部接口请求。即使最终答案还没准备好，前端也可以先展示「正在检索资料」「正在调用工具」「正在生成总结」。流处理要做的，就是把这段长过程转换成连续事件，让消费端边接收数据边更新状态。
...
浏览器端主要处理三件事，发起请求、分段读取响应体，以及在用户取消时停止请求。`fetch` 是发起请求的入口。普通请求里，我们经常这样用：
...
这表示等响应体完整回来后，再一次性解析 JSON。流式请求不能这样处理，因为响应体可能持续返回，甚至很久都不会结束。这时要读取 `response.body`：
...
如果要返回流，Route Handler 可以直接返回 `Response`，把 `ReadableStream` 作为响应体。在 `start` 中调用 `controller.enqueue()`，就能持续写入数据：
...
如果通过 LangChain 调模型，通常会拿到可以直接 `for await` 消费的流：
...
这种方式对业务代码更友好，不需要直接处理底层字节。Next.js 中转层只要把 LangChain 事件转换成统一的业务事件，再返回给浏览器即可。有些 Node.js 生态库仍然返回 Node.js `Readable`，这属于兼容问题，不是本文主线。遇到这种情况时，先消费库返回的流，再转换成前端稳定可理解的业务事件。

## SSE 的端到端链路

SSE 是承载事件的传输格式。大模型接口通常用它返回增量，Next.js 中转层也会把响应包装成 SSE 供浏览器消费。应用行为由事件的业务含义决定，包括文本增量、工具调用、状态变化和结束信号。

SSE 在整条业务链路中的位置如下。模型或 Agent 产生事件，中转层负责适配，浏览器再把业务事件映射成 UI 状态。
...
只要这条链路稳定，底层使用 SSE、NDJSON 或其它流式文本格式，都只是传输细节。

## 流处理的分层

实现流处理时，可以把数据变化拆成五个层次。
...
这里有两次不能省略的转换。chunk 要先按协议边界拼装成完整事件，完整协议事件再经过字段映射，才会成为业务事件。
...
把这些部分抽到 `lib/sse.ts`，浏览器和服务端的示例就可以直接复用。这组工具只处理本文约定的 `event` 和 JSON `data` 字段，不是覆盖 SSE 全部字段的通用解析器。
...
后面的示例会直接使用这三个函数，把业务代码限定在「读取上游事件、转换字段、写出业务事件」这条主线上。
...
这种方式最直接。浏览器发起请求后，用 `parseSSE` 迭代上游 SSE 事件。
...
它的链路短，适合 demo 和本地验证。生产环境通常不采用这种方式，原因主要有两个。
...
更常见的方式是浏览器只调用自己的 Next.js 接口，再由 Route Handler 请求大模型接口。
...
这类架构适合作为多数业务的基础版本，既保留流式体验，也把安全控制和协议适配放在服务端。接入多步 Agent、工具调用和检索结果后，可以再用 SDK 或 LangChain 收敛供应商调用与事件解析代码。

## 服务端通过 OpenAI SDK 接入模型

下面仍以 Next.js App Router 的 Route Handler 作为 HTTP 入口。OpenAI SDK 的调用属于服务端逻辑，也可以迁移到独立的 Node.js 服务。

如果不想手写请求体、鉴权头和分块解析，可以使用 OpenAI 官方 Node SDK。Responses API 会把 SSE 解析成带类型的语义事件，业务代码只需要处理关心的事件类型。
...
SDK 把流式响应包装成 `AsyncIterable`，并通过 `response.output_text.delta` 等事件类型区分文本、工具参数和生命周期变化。这个示例把文本增量转换成应用的 `message` 事件，并将失败事件交给统一错误处理。如果需要工具调用，可以继续映射相应的 Responses 流事件，中转层以下仍然使用同一套业务事件协议。

## 服务端通过 LangChain 接入模型

下面仍以 Next.js App Router 的 Route Handler 作为 HTTP 入口。LangChain 的模型调用属于服务端逻辑，也可以迁移到独立的 Node.js 服务。

前两种写法都直接依赖供应商 API。如果应用需要统一模型调用方式，或继续接入工具与多步 Agent，可以引入 LangChain。下面用 `ChatOpenAI` 演示最基础的模型文本流。
...
LangChain 提供统一的模型调用接口，Next.js 负责把模型返回的文本 chunk 包装成浏览器可以消费的 SSE。切换供应商时仍需更换对应的集成类和配置，业务层的流式消费方式可以保持一致。
...
有了 Next.js 中转层后，浏览器无需关心后端通过原始 API、SDK 还是 LangChain 调用模型，只处理业务接口约定的事件。
...
前端消费时，不要把所有事件都当成文本拼接。以文本流接口为例，只处理 `message`、`error`、`done` 三类事件。
...
如果后端返回 SSE，浏览器可以用 `EventSource`，也可以用 `fetch` 读取流。`EventSource` 只建立服务端到浏览器的单向 GET 连接，浏览器会按 `event:` 名称自动分发事件。示例只在 URL 中传递服务端保存的会话 ID，不把用户输入直接放进 URL 和访问日志：
...
`EventSource` 建立连接后，浏览器会保持一个到 SSE 地址的 GET 请求。如果连接因为网络抖动、代理断开或服务端临时异常而中断，浏览器会触发连接错误事件。只要代码没有调用 `source.close()`，浏览器就会等待一段时间后重新请求同一个地址。
...
`EventSource` 只支持 GET，也不方便携带自定义请求头。Chat 或 Agent 接口通常需要 POST body、复杂鉴权和请求上下文，这时用 `fetch` + `parseSSE` 更灵活。
...
如果老项目只能用 `XMLHttpRequest`，也可以在 `onprogress` 中读取新增的 `responseText`，再按 SSE 的空行边界切分事件。
...
`parseSSE`、`writeSSE` 和 `createSSEResponse` 封装了字节、协议与生命周期处理，业务代码仍需处理部署和异常边界。反向代理、负载均衡器或托管平台可能缓冲响应，导致服务端持续写入，浏览器却迟迟收不到事件。部署时要确认整条链路支持流式传输，并关闭代理缓冲。以 Nginx 为例，可以配置 `X-Accel-Buffering: no`。如果工具调用期间长时间没有业务事件，还需要发送心跳，并检查各层的空闲超时设置。SSE 注释行不会触发业务事件，适合用作心跳，例如 `: ping\n\n`。
...
流式接口的错误不一定只发生在请求开始阶段。请求校验或鉴权失败等已知错误应在流开始前通过 HTTP 状态码返回。响应开始传输后，HTTP 状态码已经发出，上游模型断开或工具超时等错误只能转换成前端可理解的事件。
