...
Pi Agent 是一套面向编码任务的 Agent Harness。它既提供可直接使用的编码 Agent，也把模型通信、Agent Runtime、编码产品和终端界面拆成独立模块，供开发者组合自己的 Agent 产品。

模型负责理解任务、生成响应和提出工具调用；Harness 负责组装上下文、驱动循环、执行工具并维护状态。单次模型调用由此被组织成可持续、可观察、可干预的任务执行过程。

本系列把 Pi 作为一个可运行、可阅读的 Harness 样本，结合源码追踪各模块如何协作，进而理解 Harness Engineering，并将这些机制用于自己的 Agent 产品或基于 Pi 构建的新产品。

## 为什么值得研究

Pi 能完成真实编码任务，也把关键机制放在了容易阅读和替换的位置：

- **核心极简**。模型调用、工具调度、运行状态和事件流集中在少数核心边界内，可以沿着一次任务的调用链理解 Agent 如何运行；
- **源码清晰**。模型通信、Agent Runtime、编码产品和终端 UI 分属不同包，抽象边界能够直接对应到代码；
- **能力可扩展**。模型提供商、工具、Skills、Extensions 和界面都可以替换或组合，产品能力不必全部进入核心运行时；
- **经验可迁移**。这些设计既适合分析现有 Harness，也可以复用于新的 Agent 产品。

对使用者，Pi 是一个能完成真实编码任务的终端 Agent；对开发者，它是一套可以拆开阅读、替换和重新组合的 Harness。这种双重定位使源码阅读能够直接连接真实产品问题。
