...
一定时间连续触发的事件只在最后执行一次<br/>注意事项：<br/>1. 修正 this<br/>2. 第一次是否需要立即执行
...
一段时间内只执行一次<br/>setTimeout 版本<br/>1. 事件触发不立即执行<br/>2. 事件触发停止后还会执行一次<br/>3. 异步执行
...
时间戳版本<br/>1. 事件触发立即执行<br/>2. 事件触发停止后不会再执行<br/>3. 同步执行
...
综合版<br/>1. 事件触发立即执行<br/>2. 事件触发停止后再执行一次
