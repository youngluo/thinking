...
从输入 URL 到页面渲染完成，主线可以压缩成这条链路：

`输入 URL -> 导航调度 -> 网络请求 -> 提交文档 -> 解析 HTML -> 加载子资源 -> 执行脚本 -> 样式计算 -> 布局 -> 分层 -> 绘制 -> 光栅化 -> 合成显示`

现代浏览器通常是多进程架构。下面用 Chrome 的模型来理解：
...
其中渲染进程内部最关键的是主线程、合成线程、栅格线程池。主线程最容易成为性能瓶颈，因为 HTML 解析、样式计算、布局、脚本执行大多都要经过它。

## 1、URL 导航

这一段主要发生在浏览器进程的 UI 线程里。
...
这一段主要由网络进程处理，内部通常会用到网络服务线程和工作线程池。

浏览器进程把导航请求交给网络进程。网络进程负责缓存判断、DNS 解析、连接建立、请求发送和响应接收。

### 2.1、缓存策略

这里主要还是网络进程在处理。

网络进程发请求前，会先判断浏览器缓存是否可用。缓存判断不只发生在 HTML 主文档，也发生在 CSS、JavaScript、图片、字体等子资源上。
...
这里主要由网络进程处理，必要时会调用系统 DNS 能力。

缓存未命中时，网络进程会做 DNS 解析，把域名解析成 IP 地址。查询顺序通常包括浏览器缓存、系统缓存、hosts 文件、本地 DNS 服务和权威 DNS。
...
如果是普通 HTTP，网络进程通常会先建立 TCP 连接。TCP 三次握手可以简化理解为：
...
HTTP/1.1 可以复用 TCP 连接，但同一连接上的请求容易受队头阻塞影响。HTTP/2 仍然基于 TCP，但在一个连接上做多路复用，可以同时承载多个请求。HTTP/3 基于 QUIC，运行在 UDP 之上，把传输和加密握手整合得更紧，弱网下恢复能力也更好。
...
这里主要由网络进程处理，证书校验也在这一层完成。

HTTPS 在 HTTP/1.1 和 HTTP/2 下，通常是先建立 TCP 连接，再进行 TLS 握手。TLS 握手主要做四件事：
...
如果命中 TLS 会话复用，浏览器可以减少握手成本。HTTP/3 使用 QUIC，不再走 TCP 三次握手，而是把连接建立和 TLS 1.3 加密协商整合在一起，通常可以更快完成建连。
...
连接建好后，网络进程组装 HTTP 请求，包括请求行、请求头、Cookie、缓存控制信息和可选请求体。请求会经过代理、网关、CDN 或负载均衡，最终到达目标服务器。

服务端收到请求后，会执行路由匹配、鉴权、业务逻辑、数据查询和模板渲染或接口响应，然后返回 HTTP 响应。响应回到浏览器后，就进入下一节的接收响应阶段。
...
```http
Set-Cookie: token=xxx; Path=/; Domain=www.example.com; HttpOnly; Secure; SameSite=Lax
```
...
如果响应是下载类型，就交给下载管理器。如果是 HTML，网络进程会把响应数据转交给渲染进程。如果响应体被压缩，网络层会先解压或边解压边传输。
...
这里主要发生在渲染进程主线程里，预加载扫描器会在后台帮忙提前发现资源。
...
<link rel="preload" href="/assets/rest.css" as="style" onload="this.rel='stylesheet'" />
```
...
这一步是渲染进程发现资源，网络进程去下载。

HTML 解析发现 CSS、JavaScript、图片、字体、视频等资源后，会把请求交给网络进程。网络进程会根据优先级、缓存状态、协议能力和连接复用情况调度下载。
...
这里主要跑在渲染进程主线程上，JavaScript 引擎执行页面脚本也在这一条线程里。
...
每个节点都会得到一份计算后的样式，也就是 computed style。样式计算会处理选择器匹配、继承、层叠优先级和默认样式。
...
图层树由渲染进程主线程生成，后续管理更多交给合成线程。

布局完成后，浏览器会生成图层树。普通元素会归入父节点所在图层；拥有复杂效果或合成收益的元素，可能被提升为单独图层。

常见的分层原因包括 3D 变换、页面滚动、`z-index`、视频、canvas，以及需要裁剪的区域。分层可以让后续合成更高效，但图层过多也会增加内存和合成成本。
...
这里主要由渲染进程的合成线程和栅格线程池处理，必要时会借助 GPU 进程。

合成线程会把图层切成图块，并把图块交给栅格线程转换成位图。靠近视口的图块通常会优先光栅化，必要时还会使用 GPU 加速。

光栅化完成后，合成线程会收集 draw quads，也就是最终合成需要的图块信息。
...
所以一次页面打开，可以理解成三条线同时推进：浏览器进程负责导航和页面管理，网络进程负责拿资源，渲染进程负责把资源变成像素；它们互相协作，最终形成用户看到并能操作的页面。
