...
`key` 和 `type` 匹配时复用旧 Fiber，否则创建新的 Fiber。未复用的旧 Fiber 标记为删除；复用后位置变化的 Fiber 标记为移动。具体的单节点和多节点 diff 过程，见[《React Reconcile 实现》](<./Reconcile 实现.md>)。
