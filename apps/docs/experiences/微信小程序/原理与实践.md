...
小程序运行环境由四部分组成：`Native`、逻辑层、渲染层、基础库。
...
`Native` 是微信客户端提供宿主能力的那一层。逻辑层是跑用户 JS 的引擎，iOS 上通常是 JavaScriptCore，Android 侧由客户端内置的 JS 引擎承载。渲染层主要承载在 WebView 上，负责 WXML/WXSS 的绘制；少数原生组件（`map`、`video`、`canvas` 等）由客户端原生层直接渲染，渲染层在 WebView 中为它们预留位置并通过 JSBridge 触发 Native 绘制。基础库（`wx`）是微信自己实现的一套运行时，一部分以 JS 代码形式注入到逻辑层，一部分以原生代码形式驻留在客户端里。

这四部分都不归用户代码控制。开发者写下的 JS 只会跑在逻辑层；样式和模板经过编译后送进渲染层；任何越界操作（DOM 操作、动态执行代码）都会失败。

### 基础库与版本约束

小程序的可用能力，由**客户端版本、基础库版本、用户代码版本**三者共同决定。基础库由微信发布，可能内置在某个客户端版本里，也可能在用户首次打开小程序时按需下载；用户代码版本由开发者上传。三者对本次运行环境的约束关系大致如下：
...
逻辑层负责执行用户 JS，包括根入口 `app.js`、页面 `page.js`、自定义组件的 `.js`，以及用 `new Worker()` 创建的 Worker。它管理的是 `Page` / `Component` 的 `data` 和生命周期，不能直接操作视图；影响 UI 的主要出口是 `setData`，外部输入则来自事件回调和生命周期回调。
...
- `setData` 触发的更新流程大致是“逻辑层虚拟 DOM 更新 → 跨线程消息 → 视图层更新与页面渲染”，跨线程消息走 JSBridge，渲染层内部更新（虚拟 DOM diff、Exparser 节点更新）在 WebView 内完成。
...
- **逻辑层虚拟 DOM 更新**：框架会在逻辑层遍历并更新对应的虚拟 DOM 树，触发相关组件生命周期和 `observer` 等逻辑；
- **跨线程消息**：本次变更的数据会从逻辑层传到视图层，依赖 JSBridge。数据量越大、调用越频繁，通信和调度成本越明显；
- **视图层更新与页面渲染**：视图层收到数据后更新自身的虚拟 DOM 树和真实 DOM，并触发页面渲染更新。

`setData` 回调表示这次数据更新流程已完成，但不等同于屏幕像素已稳定。后续布局、图片解码、原生组件更新等工作，仍可能在回调之后继续发生。

:::info 特别说明
逻辑层和渲染层都有“虚拟 DOM”，但含义不同：

- **逻辑层**：`data` 字段按组件实例组织的数据依赖树，没有 DOM/BOM 概念，用来计算哪些数据路径变了、需要触发哪些 observer 和生命周期；
- **渲染层**：WXML 编译产物的 UI 节点抽象树，类似 React vDOM，收到数据后用 diff → patch 驱动真实 DOM 更新。
...
- **页面渲染与原生组件更新**：真实 DOM 更新后还会触发布局、绘制等浏览器渲染成本；涉及原生组件时，还可能继续产生原生层更新成本。
...
小程序不是普通 Web 页面套了一层客户端壳，而是一套由 `Native`、逻辑层、渲染层和基础库共同组成的运行环境。它用双线程模型隔离 JS 和 UI，用基础库收口宿主能力，也把版本兼容、跨线程通信、启动链路和发布治理变成开发者必须面对的工程约束。

理解这套模型之后，前面的工程取舍就有了依据：

- 启动不是单个生命周期函数的问题，而是资源准备、逻辑层初始化、渲染层初始化和首屏依赖共同决定的链路；
- `setData` 不是普通赋值，而是一次跨线程数据更新，优化重点在体积、频率和影响范围；
- 分包、兼容、监控和发布流程要提前设计，让问题尽量在发布前暴露，发布后也能追踪。

把原理看清楚，不是为了记住更多概念，而是为了在遇到性能、兼容或线上问题时，能知道该沿着哪条链路定位、取舍和优化。
