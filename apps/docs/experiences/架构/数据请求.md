...
flowchart TD
    classDef default fill:#7fa3ff29,stroke:#07f,stroke-width:1px
    classDef fail fill:#ffcdd2,stroke:#b71c1c,stroke-width:1px
    classDef ok fill:#c8e6c9,stroke:#1b5e20,stroke-width:1px

    A["发起请求<br/>携带 Access Token"] --> B{"响应码"}
    B -- "200" --> C["返回数据"]
    B -- "401" --> D{"isRefreshing?"}
    D -- "true" --> E["请求入队<br/>挂起等待"]
    D -- "false" --> F["isRefreshing=true<br/>调刷新接口"]
    F --> G{"刷新结果"}
    G -- "成功" --> H["存新 token<br/>唤醒队列"]:::ok
    G -- "失败" --> I["清 token<br/>跳登录"]:::fail
    H --> J["新 token<br/>重试原请求"]
    E --> J
    J --> C
    I -.-> K["结束"]
    C -.-> K
