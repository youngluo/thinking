...
本文是[《微前端：架构决策与运行原理》](./架构决策与运行原理.md)的实践篇。原理篇讨论为什么引入微前端，以及应用加载、沙箱、样式隔离和通信契约如何工作；本文只关注如何用 qiankun 2.x 把这些边界落到项目中。
...
前面的完整 `start` 配置已经启用 Proxy 沙箱和 `experimentalStyleIsolation`，这里只说明 qiankun 配置如何映射到项目边界。沙箱与样式隔离的工作原理见[《微前端：架构决策与运行原理》](./架构决策与运行原理.md)。
